...
// Sidebar links. Only Schedule Management has a page so far — the rest are
// placeholders until their own branches are merged, so they render as plain
// (non-link) rows. `icon` maps to a .admin-nav-icon--* class that supplies
// the mask image (see ScheduleManagement.css).
const NAV_ITEMS = [
  { label: "Dashboard", icon: "dashboard", to: null },
...
const MOVIES = [
  { id: "white-chicks", title: "White Chicks", genre: "Comedy", poster: posterWhiteChicks },
  { id: "mean-girls", title: "Mean Girls", genre: "Comedy/Drama", poster: posterMeanGirls },
  { id: "disclosure-day", title: "Disclosure Day", genre: "Sci-Fi/Thriller", poster: posterDisclosureDay },
  { id: "toy-story-5", title: "Toy Story 5", genre: "Animation/Family", poster: posterToyStory },
  { id: "devil-wears-prada-2", title: "The Devil Wears Prada 2", genre: "Comedy/Drama", poster: posterDevilWearsPrada },
];

const CINEMAS = ["Cinema 1", "Cinema 2", "Cinema 3"];
const CINEMA_OPTIONS = CINEMAS.map((cinema) => ({ value: cinema, label: cinema }));
const MOVIE_OPTIONS = MOVIES.map((movie) => ({ value: movie.id, label: movie.title, thumbnail: movie.poster }));
const MOVIE_FILTER_OPTIONS = [{ value: "all", label: "All Movies" }, ...MOVIE_OPTIONS];
...
const INITIAL_SCHEDULE = [
  { id: 1, movieId: "devil-wears-prada-2", datetime: "2026-06-06T10:00", cinema: "Cinema 1", seatsTotal: 50, seatsBooked: 18 },
  { id: 2, movieId: "mean-girls", datetime: "2026-06-06T10:30", cinema: "Cinema 1", seatsTotal: 50, seatsBooked: 30 },
  { id: 3, movieId: "mean-girls", datetime: "2026-06-06T19:30", cinema: "Cinema 3", seatsTotal: 50, seatsBooked: 50 },
  { id: 4, movieId: "toy-story-5", datetime: "2026-06-07T18:30", cinema: "Cinema 1", seatsTotal: 50, seatsBooked: 22 },
];
...
function ScheduleManagement() {
  ...
  const [editForm, setEditForm] = useState({ datetime: "", cinema: CINEMAS[0] });
  ...
  const handleAddSubmit = (event) => {
    ...
    setSchedule((prev) => [
      ...prev,
      { id: Date.now(), movieId: addForm.movieId, datetime: addForm.datetime, cinema: addForm.cinema, seatsTotal: 50, seatsBooked: 0 },
    ]);
    setIsAddOpen(false);
  };
  ...
  const handleRescheduleSubmit = (event) => {
    ...
    setSchedule((prev) =>
      prev.map((row) => (row.id === editingId ? { ...row, datetime: editForm.datetime, cinema: editForm.cinema } : row))
    );
    setEditingId(null);
  };

  const editingRow = editingId !== null ? schedule.find((row) => row.id === editingId) : null;
  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="admin-main">
        ...
        <section className="schedule-filters" aria-label="Filters">
          <div className="filter-field">
            <span>Movie</span>
            <Dropdown options={MOVIE_FILTER_OPTIONS} value={movieFilter} onChange={setMovieFilter} ariaLabel="Filter by movie" />
          </div>
                  ...
                  <tr key={row.id}>
                    <td data-label="Movie">
                      <div className="schedule-movie">
                        <img className="schedule-poster" src={movie.poster} alt="" />
                        <div>
                          <p className="schedule-movie-title">{movie.title}</p>
                          <p className="schedule-movie-genre">{movie.genre}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td data-label="Cinema">{row.cinema}</td>
                    <td data-label="Seats">
                      <div className="seat-bar" title={`${row.seatsBooked}/${row.seatsTotal} seats booked`}>
                        <div className="seat-bar-fill" style={{ width: `${percent}%` }} />
                      </div>
                    </td>
      ...
      {isAddOpen && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="add-showtime-title">
          <form className="modal-card" onSubmit={handleAddSubmit}>
            <h2 id="add-showtime-title">Add showtime</h2>
            <p className="modal-subtitle">Fill in details for the new schedule entry.</p>
            ...
            <div className="modal-field">
              ...
              <Dropdown
                ...
                onChange={(movieId) => setAddForm((form) => ({ ...form, movieId }))}
                ariaLabel="Movie"
              />
            </div>
            ...
            <label className="modal-field">
              ...
              <input
                ...
                onChange={(event) => setAddForm((form) => ({ ...form, datetime: event.target.value }))}
              />
            </label>
            ...
            <div className="modal-field">
              ...
              <Dropdown
                ...
                onChange={(cinema) => setAddForm((form) => ({ ...form, cinema }))}
                ariaLabel="Cinema"
              />
            </div>

            <div className="modal-actions">
              <button type="button" className="modal-btn modal-btn--ghost" onClick={() => setIsAddOpen(false)}>
                Cancel
              </button>
      ...
      {editingRow && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="reschedule-title">
          <form className="modal-card" onSubmit={handleRescheduleSubmit}>
            <h2 id="reschedule-title">Reschedule showtime</h2>
            <p className="modal-subtitle">
              Update the date, time, or cinema for <strong>{findMovie(editingRow.movieId).title}</strong>.
            </p>
            ...
            <div className="modal-field">
              ...
              <Dropdown
                ...
                onChange={(cinema) => setEditForm((form) => ({ ...form, cinema }))}
                ariaLabel="Cinema"
              />
            </div>

            <p className="modal-notice">
              Rescheduling this will automatically update all reserved tickets. Ticket holders will be notified via
              email.
            </p>

            <div className="modal-actions">
              <button type="button" className="modal-btn modal-btn--ghost" onClick={() => setEditingId(null)}>
                Cancel
              </button>
              <button type="submit" className="modal-btn modal-btn--primary">
                Save changes
      ...
      {deletingRow && (
        <div className="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="delete-title">
          <div className="modal-card">
            ...
            <p className="modal-subtitle">
              You're about to delete <strong>{findMovie(deletingRow.movieId).title}</strong> on{" "}
              {formatDatetime(deletingRow.datetime)}.
            </p>

            <p className="modal-warning">
              This action cannot be undone. Any bookings for this showtime may also be affected.
            </p>

            <div className="modal-actions">
              <button type="button" className="modal-btn modal-btn--ghost" onClick={() => setDeletingId(null)}>
                Cancel
              </button>
              <button type="button" className="modal-btn modal-btn--primary" onClick={handleDeleteConfirm}>
                Delete showtime
              </button>
            </div>
          </div>
